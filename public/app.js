(function(){
  ...
  function makeDeck(k,seed){const base=TEMPLATES[k];const deck=[];const rng=typeof seed==='number'?seededRng(seed):Math.random;const pool=[...base];while(deck.length<20){const r=Math.floor(rng()*pool.length);deck.push(C(pool[r]))}return typeof seed==='number'?deck:shuffle(deck)}
  ...
  $$('.deckbtn').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    if(!(window.NET_STATE && window.NET_STATE.online)){
      if(HUMAN_DECKS.includes(pick)){S.aiDeckChoice=ANIMAL_DECKS[Math.floor(Math.random()*ANIMAL_DECKS.length)]}else{S.aiDeckChoice=HUMAN_DECKS[Math.floor(Math.random()*HUMAN_DECKS.length)]}
    }
  ...
  Object.assign(window,{S,makeDeck,shuffle,draw,newTurn,renderAll,aiTurn,playFromHand,endTurn,attackCard,attackFace});
})();
