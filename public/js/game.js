...
const KW={P:'Protetor',F:'Furioso'},BC={D1:'draw1',H2:'heal2',P1:'ping1',BR1:'buffRandom1',BA1:'buffAlliesAtk1',M1:'mana1',SM:'sacMana'};const makeCard=a=>{const[n,e,t,atk,hp,cost,tx,k=0,b=0,i]=a;return{name:n,emoji:e,tribe:t,atk,hp,cost,text:tx,kw:k?k.split('|').map(x=>KW[x]):[],battlecry:b?BC[b]:void 0,icon:i,id:uid()}};const TEMPLATES={vikings:[['Lavrador de Lança','🧔‍🌾','Viking',2,2,2,'Disciplinado','', '',0],['Camponesa Curandeira','👩‍🌾✨','Viking',2,3,3,'Entra: cura 2','', 'H2',1],['Ceifeiro Berserker','👨‍🌾⚔️','Viking',5,2,4,'Furioso','F','',2],['Escudeiro Rural','🛡️','Viking',0,3,1,'Protetor','P','',3],['Guardião da Aldeia','🛡️🌾','Viking',3,5,4,'Protetor','P','',4],['Caçador de Lobos','🏹','Viking',3,2,2,'Entra: dano 1 aleatório','', 'P1',5],['Ferreiro Rural','🔨','Viking',4,6,5,'Entra: +1/+1 aleatório','', 'BR1',6],['Chefe da Colheita','👑🌾','Viking',5,6,6,'Aliados +1 ATK','', 'BA1',7],['Druida da Colheita','🌿','Viking',1,2,2,'Entra: ganha 1 de mana','', 'M1',8],['Sacerdote do Campo','🪓🔥','Viking',2,2,3,'Entra: sacrifica um aliado e ganha o custo como mana','', 'SM',9]],animais:[['Urso Pardo','🐻','Animal',6,6,5,'Protetor','P'],['Lobo Cinzento','🐺','Animal',4,2,3,'Furioso','F'],['Javali Selvagem','🐗','Animal',3,2,2,'Impulsivo'],['Cervo Nobre','🦌','Animal',4,5,4,'Resistente'],['Coruja Sábia','🦉','Animal',1,2,1,'Entra: compre 1','', 'D1'],['Cavalo de Guerra','🐴','Animal',3,3,3,'Confiável'],['Cabra da Montanha','🐐','Animal',2,3,2,'Protetor','P'],['Águia do Norte','🦅','Animal',5,3,4,'Veloz'],['Urso Polar','🐻‍❄️','Animal',7,7,6,'Gigante'],['Serpente do Mar','🐍','Animal',8,7,7,'Colosso']],pescadores:[['Grumete do Fiorde','👦🎣','Viking',1,1,1,'Aprendiz'],['Pescador do Fiorde','🧔‍♂️🎣','Viking',2,3,2,'Veterano'],['Arpoador Nórdico','🪝','Viking',3,2,2,'Entra: dano 1 aleatório','', 'P1'],['Curandeira do Sal','🧂✨','Viking',2,3,3,'Entra: cura 2','', 'H2'],['Vigia do Farol','🗼🛡️','Viking',2,5,4,'Protetor','P'],['Ferreiro Naval','⚓️🔨','Viking',4,5,5,'Entra: +1/+1 aleatório','', 'BR1'],['Capitão da Pesca','👑🎣','Viking',5,6,6,'Aliados +1 ATK','', 'BA1'],['Remador Ágil','🚣','Viking',4,2,3,'Furioso','F'],['Místico das Marés','🔮🌊','Viking',1,2,2,'Entra: ganha 1 de mana','', 'M1'],['Sacrifício do Mar','🪓🌊','Viking',2,2,3,'Entra: sacrifica um aliado e ganha o custo como mana','', 'SM']],floresta:[['Urso Negro','🐻','Animal',5,5,5,'Protetor','P'],['Lobo da Mata','🐺','Animal',4,2,3,'Furioso','F'],['Javali da Floresta','🐗','Animal',3,2,2,'Impulsivo'],['Cervo Vermelho','🦌','Animal',4,5,4,'Resistente'],['Coruja Sábia','🦉','Animal',1,2,1,'Entra: compre 1','', 'D1'],['Raposa Ágil','🦊','Animal',3,3,3,'Veloz'],['Bisonte das Colinas','🐂','Animal',6,6,6,'Imponente'],['Serpente do Bosque','🐍','Animal',5,4,4,'Silenciosa'],['Ent Ancião','🌳','Animal',2,4,3,'Entra: ganha 1 de mana','', 'M1'],['Xamã do Bosque','🪓🌲','Animal',2,2,3,'Entra: sacrifica um aliado e ganha o custo como mana','', 'SM']]};const HUMAN=['vikings','pescadores'],BEAST=['animais','floresta'];
...
const VIKING_CHAR_SHEET='https://videos.openai.com/vg-assets/assets%2Ftask_01k32xw6bhfxatqs3evxmptrds%2F1755666416_img_1.webp?st=2025-08-20T03%3A17%3A52Z&se=2025-08-26T04%3A17%3A52Z&sks=b&skt=2025-08-20T03%3A17%3A52Z&ske=2025-08-26T04%3A17%3A52Z&sktid=a48cca56-e6da-484e-a814-9c849652bcb3&skoid=8ebb0df1-a278-4e2e-9c20-f2d373479b3a&skv=2019-02-02&sv=2018-11-09&sr=b&sp=r&spr=https%2Chttp&sig=8%2Bisb3EsmtRseqBSReK2IiNlGwsqKxHKGeqSZMQd2M0%3D&az=oaivgprodscus';
const VIKING_TOOL_SHEET='https://videos.openai.com/vg-assets/assets%2Ftask_01k32xw6bhfxatqs3evxmptrds%2F1755666416_img_2.webp?st=2025-08-20T03%3A17%3A52Z&se=2025-08-26T04%3A17%3A52Z&sks=b&skt=2025-08-20T03%3A17%3A52Z&ske=2025-08-26T04%3A17%3A52Z&sktid=a48cca56-e6da-484e-a814-9c849652bcb3&skoid=8ebb0df1-a278-4e2e-9c20-f2d373479b3a&skv=2019-02-02&sv=2018-11-09&sr=b&sp=r&spr=https%2Chttp&sig=8%2Bisb3EsmtRseqBSReK2IiNlGwsqKxHKGeqSZMQd2M0%3D&az=oaivgprodscus';
...
function createProjection(container,idx){
  ...
  const sheet=idx>=9?VIKING_TOOL_SHEET:VIKING_CHAR_SHEET;
  loader.load(sheet,tex=>{
    tex.magFilter=THREE.NearestFilter;tex.minFilter=THREE.NearestFilter;
    const group=new THREE.Group();
    const tiles=3;const size=1/tiles;const j=idx>=9?idx-9:idx;
    const col=j%tiles,row=Math.floor(j/tiles);
    for(let i=0;i<5;i++){
      const geo=new THREE.PlaneGeometry(100,100);
      const uvs=geo.attributes.uv.array;
      const x0=col*size,y0=1-(row+1)*size,x1=x0+size,y1=y0+size;
      uvs[0]=x0;uvs[1]=y0;uvs[2]=x1;uvs[3]=y0;uvs[4]=x1;uvs[5]=y1;uvs[6]=x0;uvs[7]=y1;
...
function cardNode(c,owner,onBoard=false){
  ...
  d.dataset.id=c.id;
  const art=(c.deck==='vikings')?(!onBoard?`<div class="vikings-icon vik-icon-${c.icon}"></div>`:''):c.emoji;
  const manaDots='<span class="mana-dot"></span>'.repeat(c.cost);
  const kwSpans=(c.kw||[]).map(k=>`<span class='keyword' data-tip='${k==='Protetor'?'Enquanto houver Protetor ou carta em Defesa do lado do defensor, ataques devem mirá-los.':(k==='Furioso'?'Pode atacar no turno em que é jogada.':'')}' >${k}</span>`).join(' ');
  let text=c.text||'';
  if((c.kw||[]).includes(text)) text='';
  d.innerHTML=`<div class="bg bg-${c.deck||'default'}"></div>
  <div class="name-bar">${c.name}</div>
  <div class="cost-bar">${c.stance?`<span class=\"badge ${c.stance==='defense'?'def':'atk'}\">${c.stance==='defense'?'DEFESA':'ATAQUE'}</span>`:''}<div class="mana-row">${manaDots}<span class="mana-num">${c.cost}</span></div></div>
  <div class="art">${art}</div>
  <div class="text">${kwSpans}${text?(' '+text):''}</div>
...
function discardHand(side){const hand=side==='player'?G.playerHand:G.aiHand;const disc=side==='player'?G.playerDiscard:G.aiDiscard;if(hand.length){disc.push(...hand.splice(0));if(side==='player')els.discardCount.textContent=G.playerDiscard.length}}
