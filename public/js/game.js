(function () {
  ...
  function initAudio() {
    ...
    if (!actx) {
      ...
      master.gain.value = 0.18 * sfxVolume;
  ...
  const pickEnemyName = (deck, boss = false) => {
    const pool = (window.ENEMY_NAMES || {})[deck] || [];
  ...
  function startMenuMusic(preset) {
    ...
    musicBase = musicPreset === 'combat' ? 0.22 : 0.18;
    musicGain.gain.exponentialRampToValueAtTime(
      musicBase * musicVolume,
      actx.currentTime + 0.4,
    );
    const beat = 60 / P.bpm,
    ...
    const schedule = () => {
      ...
      for (let i = 0; i < steps; i++) {
        ...
        g.gain.exponentialRampToValueAtTime(
          (musicPreset === 'combat' ? 0.13 : 0.11) * musicVolume,
          t + i * beat + 0.01,
        );
      ...
      for (let i = 0; i < steps; i += 2) {
        ...
        g.gain.setValueAtTime(0.0001, t + i * beat);
        g.gain.exponentialRampToValueAtTime(
          (musicPreset === 'combat' ? 0.1 : 0.09) * musicVolume,
          t + i * beat + 0.01,
        );
  ...
  function setSfxVolume(v) {
    ...
    if (master) master.gain.value = sfxMuted ? 0 : 0.18 * sfxVolume;
  }
  ...
  const KW = { P: 'Protetor', F: 'Furioso', A: 'Absorver', M: 'Mutável' },
    KW_TIPS = {
      ...
      Furioso: 'Pode atacar no turno em que é jogada.',
      Absorver: 'Ao entrar, copia uma palavra-chave de um aliado.',
      Mutável: 'No fim do turno, troca ATK e HP.',
    },
    BC = {
      D1: 'draw1',
      H2: 'heal2',
    ...
    BC_NAMES = {
      ...
      ping1: 'Golpe',
      buffRandom1: 'Benção',
      buffAlliesAtk1: 'Comando',
      mana1: 'Canalizar',
      sacMana: 'Sacrifício',
    },
    BC_TIPS = {
      draw1: 'Compra 1 carta ao entrar',
      heal2: 'Cura 2 de um aliado ao entrar',
      ping1: 'Causa 1 de dano aleatório ao entrar',
      buffRandom1: 'Concede +1/+1 a um aliado aleatório ao entrar',
      buffAlliesAtk1: 'Aliados ganham +1 ATK',
      mana1: 'Ganha 1 de mana ao entrar',
      sacMana: 'Sacrifica um aliado e ganha mana igual ao custo',
    };
  const normalizeCardName = (f) =>
    f
      .replace(/\.[^.]+$/, '')
      .replace(/^nb[_-]?/i, '')
      .replace(/^\d+[_-]?/, '')
  ...
  const makeCard = (a) => {
    const [n, e, t, atk, hp, cost, tx, k = 0, b = 0, i] = a;
    let name = n;
    if (!name && typeof i === 'string') name = normalizeCardName(i);
  ...
  const DECK_IMAGES = {
    vikings: [
      '1_Guerreiro_Loiro',
      '2_Guerreiro_Esqueleto',
      '3_Guerreiro_Rubro',
      '4_Mago_Elder',
      '5_Raider_Mascara',
      '6_Guerreiro_Machado',
      '7_Sombras_Encapuzado',
      '8_Guerreiro_Espada',
      '9_Raider_Mascara_Sombra',
      '10_Mago_Elder_Sombra',
    ],
    pescadores: [
      '1_Fogueira_Viking',
      '2_Mistico_Encapuzado',
      '3_Drakkar',
      '4_Guerreiro_do_Escudo',
      '5_Estandarte_do_Cla',
      '6_Guerreiro_das_Runas',
      '7_Guardiao_do_Machado',
      '8_Batalhador_Duplo',
      '9_Navegador',
      '10_Batalhador',
    ],
  ...
  function deriveStatsFromName(name) {
    const n = name.toLowerCase();
    let atk = 3,
      hp = 3,
      kw = '',
    ...
    if (/guard/i.test(n)) {
      ...
      kw = 'P';
      text = 'Protetor';
    } else if (/mago|mistico/.test(n)) {
      atk = 2;
      hp = 3;
      bc = 'H2';
      text = 'Entra: cura 2';
    } else if (/guerreiro|batalhador|raider|lobo|raposa/.test(n)) {
      atk = 4;
      hp = 2;
      kw = 'F';
      text = 'Furioso';
    } else if (/fogueira|estandarte/.test(n)) {
      atk = 1;
      hp = 1;
      bc = 'BR1';
      text = 'Entra: +1/+1 aleatório';
    } else if (/coruja/.test(n)) {
      atk = 1;
      hp = 2;
      bc = 'D1';
      text = 'Entra: compre 1';
    } else if (/serpente/.test(n)) {
      atk = 5;
      hp = 4;
    } else if (/alce|urso|bode|cervo/.test(n)) {
      atk = 4;
      hp = 5;
      kw = 'P';
      text = 'Protetor';
    }
    return { atk, hp, kw, bc, text };
  }
  function buildDeck(key) {
    const tribe =
      key === 'vikings' || key === 'pescadores' ? 'Viking' : 'Animal';
    return (DECK_IMAGES[key] || []).map((fn) => {
      const name = normalizeCardName(fn);
      const s = deriveStatsFromName(name);
      const cost = Math.max(1, Math.round((s.atk + s.hp) / 2));
      return [name, '', tribe, s.atk, s.hp, cost, s.text, s.kw, s.bc, fn];
    });
  }
  const TEMPLATES = {
    vikings: [
  ...
  class StoryMode {
    ...
    nextRound() {
      ...
      this.currentEncounter = isBoss ? 'boss' : isElite ? 'elite' : 'normal';
    ...
    handleVictory() {
      ...
      return { leveled, rewards: this.rewardOptions(), goldGain };
    }
    rewardOptions() {
  ...
  function updateCardSize() {
    ...
    const marginY = (e) => {
      ...
      return parseFloat(st.marginTop) + parseFloat(st.marginBottom);
    };
    const outer = (e) => (e ? e.offsetHeight + marginY(e) : 0);
    const pad = (() => {
      const st = getComputedStyle(wrap);
      return parseFloat(st.paddingTop) + parseFloat(st.paddingBottom);
    })();
    const bodyPad = (() => {
      try {
    ...
    const space =
      ...
      bodyPad +
      24;
    let cardH = (vh - space) / 3;
    if (!isFinite(cardH) || cardH < 40) cardH = 40;
    const cardW = cardH * (220 / 300);
    root.style.setProperty('--card-h', cardH + 'px');
    root.style.setProperty('--card-w', cardW + 'px');
  }
  window.addEventListener('resize', updateCardSize);
  const DECK_TITLES = {
    vikings: 'Fazendeiros Vikings',
    animais: 'Bestas do Norte',
    pescadores: 'Pescadores do Fiorde',
    floresta: 'Feras da Floresta',
    convergentes: 'Convergentes da Aurora',
    custom: 'Custom',
  };
  const DECK_ASSETS = {
    vikings: {
  ...
  function toKebabLower(s) {
    return wordsOf(s).join('-').toLowerCase();
  }
  function toSnakeTitle(s) {
    const w = wordsOf(s).map(
  ...
  function nameBasedCharUrls(deck, name) {
    ...
    if (!info) return [];
    const bases = [];
    const keb = toKebabLower(name);
    if (keb) bases.push(keb);
    const snake = toSnakeTitle(name);
    if (snake) bases.push(snake);
    const snakeLower = wordsOf(name).join('_').toLowerCase();
    if (snakeLower) bases.push(snakeLower);
    const urls = [];
    bases.forEach((b) =>
  ...
  function renderPool() {
    ...
    all.forEach((raw) => {
      const name =
        raw[0] || (typeof raw[9] === 'string' ? normalizeCardName(raw[9]) : '');
      const emoji = raw[1] || '';
      const row = document.createElement('div');
      row.className = 'pitem';
      row.innerHTML = `<span class="c">${raw[5]}</span><div>${emoji} ${name}</div><button class="add">+</button>`;
  ...
  function tiltify(card, lift = !1) {
    ...
    card.addEventListener('mousemove', (e) => {
      if (card.classList.contains('chosen')) return;
      const x = (e.offsetX / w - 0.5) * 12,
        y = (e.offsetY / h - 0.5) * -12,
        ty = hov ? -20 : 0;
      card.style.transform = `translateY(${ty}px) perspective(600px) rotateX(${y}deg) rotateY(${x}deg)`;
    });
  }
  function showPopup(anchor, text) {
    const box = document.createElement('div');
    box.className = 'card-popup';
  ...
  function createProjection(container, card) {
    ...
    if (card && card.name) {
      urls = urls.concat(nameBasedCharUrls(card.deck, card.name));
    }
    if (
      card &&
      typeof card.img === 'string' &&
      card.img.trim() &&
      !card.img.includes('img/cards/')
    )
      urls.push(card.img);
    ...
    if (urls.length) {
      ...
      if (IMG_CACHE[src] && IMG_CACHE[src].complete) {
        ...
        setSrcFallback(img, urls.slice(), showFallback);
      }
      img.width = 96;
      img.height = 96;
      img.loading = 'eager';
  ...
  function isTotem(card) {
    return (
      card &&
      (card.type === 'totem' ||
        /Totem/i.test(card.subclasse || '') ||
        /Totem/i.test(card.name || ''))
    );
  }
  ...
  function cardNode(c, owner, onBoard = false) {
    ...
    (c.kw || []).forEach((k) => {
      const tip = KW_TIPS[k] || '';
      kwTags.push(`<span class='keyword' data-tip='${tip}'>${k}</span>`);
    });
    if (c.battlecry) {
      const n = BC_NAMES[c.battlecry],
        tip = BC_TIPS[c.battlecry];
    ...
    const text = c.text || '';
    const tip =
      text && !kwTags.some((s) => s.includes('>' + text.trim() + '<'))
        ? text
        : '';
  ...
  <div class="stats"><span class="gem atk">⚔️ ${c.atk}</span>${c.stance ? `<span class="stance-label ${c.stance}">${c.stance === 'defense' ? '🛡️' : '⚔️'}</span>` : ''}<span class="gem hp ${c.hp <= 2 ? 'low' : ''}">❤️ ${c.hp}</span></div>`;
    ...
    try {
      if (isTotem(c)) {
        ...
        const art = d.querySelector('.art');
        if (art) {
          art.innerHTML = `<div class="totem-icon-art">${totemIcon(c)}</div>`;
        }
  ...
  function updateMeters() {
    ...
    try {
      if (els.midMana) {
        els.midMana.textContent = `${G.playerMana}/${G.playerManaCap}`;
      }
    } catch (_) {}
  }
  function updateOpponentLabel() {
    if (!els.opponentLabel) return;
    if (window.isMultiplayer) {
      els.opponentLabel.textContent = window.opponentName
        ? ` ${window.opponentName}`
        : '';
    } else if (G.mode === 'story') {
      els.opponentLabel.textContent = '';
    } else {
      const t = DECK_TITLES[G.aiDeckChoice] || '';
      els.opponentLabel.textContent = t ? ` ${t}` : '';
    }
  }
  function renderAll() {
    ...
    G.playerHand.forEach((c) => {
      ...
      d.addEventListener('click', (e) => {
        ...
        if (isTotem(c)) {
          openTotemConfirm(
            d,
            () => window.activateTotemById(c.id, d),
            () => {},
          );
        } else {
          openStanceChooser(d, (st) => {
            animateHandCardToBoard(d, () => playFromHand(c.id, st));
          });
        }
      });
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (inner) {
            ...
            while (front && front.firstChild)
              node.insertBefore(front.firstChild, node.firstChild);
            inner.remove();
          }
          node.classList.remove('show-back');
        }, 175);
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      document.querySelectorAll('.hand .card.chosen').forEach((cn) => {
        cn.classList.remove('chosen');
        cn.style.transform = '';
      });
      cancelTargeting();
    },
  ...
  function renderBoard() {
    ...
    for (const c of G.playerBoard) {
      ...
      tiltify(d);
      const art = d.querySelector('.art');
      if (!art.querySelector('.projection')) {
        const p = document.createElement('div');
        p.className = 'projection';
        art.appendChild(p);
        createProjection(p, c);
      } else {
        const p = art.querySelector('.projection');
        if (p) createProjection(p, c);
      }
  ...
  function renderTotems() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (G.totems[i]) {
        slot.textContent = G.totems[i].icon || '🗿';
      }
  ...
  function openStanceChooser(anchor, cb, onCancel) {
    ...
    setTimeout(() => {
      const h = (ev) => {
        if (ev.target.closest('.stance-chooser') || ev.target === anchor)
          return;
  ...
  function stackHand() {
    ...
    const width = cards[0].offsetWidth,
      spread = Math.round(width * 0.75),
      overlap = width - spread;
  ...
  function startGame(opts = 'player') {
    ...
    if (G.mode === 'story') {
      ...
      log(
        `Round ${G.story.round}: ${G.currentEnemyName} (${G.story.currentEncounter})`,
      );
    ...
    els.emojiBar &&
      (els.emojiBar.style.display = window.isMultiplayer ? 'flex' : 'none');
    setDeckBacks();
    if (first === 'player') draw('player', 5);
    else draw('ai', 5);
    newTurn(true);
    renderAll();
    stopMenuMusic();
  ...
  function discardHand(side) {
    ...
    const disc = side === 'player' ? G.playerDiscard : G.aiDiscard;
    if (hand.length) {
      if (side === 'player') {
        const cards = $$('#playerHand .card');
        const pile = document.getElementById('discardPile');
        cards.forEach((c) => animateMove(c, pile));
      }
      disc.push(...hand.splice(0));
      if (side === 'player') {
        els.discardCount.textContent = G.playerDiscard.length;
  ...
  function applyTotemBuffs() {
    ...
    G.playerBoard.forEach((u) => {
      u.atk = u.baseAtk !== undefined ? u.baseAtk : u.atk;
      u.hp = u.baseHp !== undefined ? u.baseHp : u.hp;
      u.baseAtk = u.atk;
      u.baseHp = u.hp;
    });
    G.totems.forEach((t) => {
  ...
  function showEmoji(side, e) {
    const el = side === 'player' ? els.playerEmoji : els.opponentEmoji;
    if (!el) return;
    el.textContent = e;
  ...
  function newTurn(skipDraw = false, prev) {
    ...
    if (G.current === 'player') {
      if (!skipDraw) {
        if (G.playerDeck.length <= 4) {
          G.playerDeck.push(...shuffle(G.playerDiscard.splice(0)));
        }
        draw('player', 5);
      }
      ...
      if (!skipDraw) {
        if (G.aiDeck.length <= 4) {
          G.aiDeck.push(...shuffle(G.aiDiscard.splice(0)));
        }
        draw('ai', 5);
      }
  ...
  function playFromHand(id, st) {
    ...
    if (c.cost > G.playerMana || boardFull) return;
    ...
    if (c.type === 'totem') {
      if (G.totems.length >= 3) {
        ...
        const t = { name: c.name, buffs: c.buffs || { atk: 1, hp: 1 } };
        G.totems.push(t);
        applyTotemBuffs();
        log(`${c.name} ativado.`);
      }
  ...
  function summon(side, c, st = 'attack', skipBC = false) {
    ...
    if (c.kw && c.kw.includes && c.kw.includes('Absorver'))
      absorbFromAlly(side, c);
      ...
      setTimeout(() => {
        ...
        try {
          ...
          Object.assign(ghost.style, {
            ...
            pointerEvents: 'none',
            transition:
              'transform .45s cubic-bezier(.2,.9,.2,1),opacity .25s ease',
          ...
          console.error('summon animation error', e);
  ...
  function applyBattlecryEffects(side, effects) {
    effects.forEach((e) => {
      if (e.type === 'heal') {
        const allies = side === 'player' ? G.playerBoard : G.aiBoard;
        const t = allies.find((x) => x.id === e.targetId);
        if (t) {
          t.hp = Math.min(t.hp + e.amount, 20);
          fxTextOnCard(t.id, '+' + e.amount, 'heal');
          particleOnCard(t.id, 'healing');
        }
      } else if (e.type === 'damage') {
        const foes = side === 'player' ? G.aiBoard : G.playerBoard;
        const t = foes.find((x) => x.id === e.targetId);
        if (t) {
          damageMinion(t, e.amount);
          particleOnCard(t.id, 'attack');
          fxTextOnCard(t.id, '-' + e.amount, 'dmg');
        }
      } else if (e.type === 'buff') {
        const allies = side === 'player' ? G.playerBoard : G.aiBoard;
        const t = allies.find((x) => x.id === e.targetId);
        if (t) {
          t.atk += e.atk;
          t.hp += e.hp;
          fxTextOnCard(t.id, '+' + e.atk + (e.hp ? '/' + e.hp : ''), 'buff');
          particleOnCard(t.id, 'magic');
        }
      } else if (e.type === 'mana') {
        if (side === 'player') {
          G.playerManaCap = clamp(G.playerManaCap + e.amount, 0, 10);
          G.playerMana = Math.min(G.playerMana + e.amount, G.playerManaCap);
        } else {
          G.aiManaCap = clamp(G.aiManaCap + e.amount, 0, 10);
          G.aiMana = Math.min(G.aiMana + e.amount, G.aiManaCap);
        }
      } else if (e.type === 'sacMana') {
        const allies = side === 'player' ? G.playerBoard : G.aiBoard;
        const discard = side === 'player' ? G.playerDiscard : G.aiDiscard;
        const t = allies.find((x) => x.id === e.targetId);
        if (t) {
          allies.splice(allies.indexOf(t), 1);
          discard.push(t);
          resetCardState(t);
          particleOnCard(t.id, 'explosion');
        }
  ...
  function showEncounterBanner(name, type = 'enemy') {
    ...
    try {
      ...
      const old = board && board.querySelector('.board-banner');
      if (old) old.remove();
    } catch (_) {}
  ...
  function encounterTransition(cb) {
    const t = document.getElementById('encounterTransition');
    if (!t) {
      cb();
      return;
    }
  ...
  function particleOnFace(side, n) {
    ...
    const anchor =
      side === 'ai' ? els.barAHP || els.aHP2 : els.barPHP || els.pHP2;
    if (!anchor) return;
    const r = anchor.getBoundingClientRect();
    screenParticle(n, r.left + r.width / 2, r.top + r.height / 2);
  }
  ...
  async function aiTurn() {
    ...
    if (skill === 2) {
      playable.sort((a, b) => b.atk + b.hp - (a.atk + a.hp));
    } else if (skill === 1) {
    ...
    function next() {
      ...
      if (legal.length) {
        let target;
        if (skill === 2) {
          target = legal.reduce((p, c) => (c.hp < p.hp ? c : p), legal[0]);
        } else {
          target = rand(legal);
        }
        attackCard(a, target);
  ...
  els.muteBtn.addEventListener('click', () => {
    ...
    musicMuted = allMuted;
    sfxMuted = allMuted;
    if (master) master.gain.value = allMuted ? 0 : 0.18 * sfxVolume;
    if (musicGain)
      musicGain.gain.value = allMuted ? 0 : musicBase * musicVolume;
    els.muteBtn.textContent = allMuted ? '🔇 Som' : '🔊 Som';
  });
  ...
  function confirmExit() {
    return G.mode === 'story'
      ? confirm('Progresso da história será perdido. Continuar?')
      : confirm('Tem certeza?');
  }
  if (els.openMenuBtn)
    els.openMenuBtn.addEventListener('click', () => {
      ...
      els.restartBtn &&
        (els.restartBtn.style.display = window.isMultiplayer
          ? 'none'
          : 'block');
    });
  if (els.closeMenuBtn)
    els.closeMenuBtn.addEventListener('click', () => {
  ...
  if (els.mainMenuBtn)
    els.mainMenuBtn.addEventListener('click', () => {
      ...
      const deck = document.getElementById('start');
      ...
      window.mpState = null;
      const custom = document.querySelector('.deckbtn[data-deck="custom"]');
      custom && (custom.style.display = '');
      if (els.startGame) {
        els.startGame.textContent = 'Jogar';
        els.startGame.disabled = true;
      }
    });
  if (els.restartBtn)
  ...
  function initCommonButtonHover() {
    ...
    $$(sel).forEach((b) => {
      if (!b.classList.contains('deckbtn')) attachButtonHover(b);
    });
  }
  ...
  if (els.saveDeck)
    els.saveDeck.addEventListener('click', () => {
      if (G.customDeck && G.customDeck.length) {
        els.deckBuilder.style.display = 'none';
        els.startGame.disabled = false;
      }
    });
  els.startGame.addEventListener('click', () => {
    if (els.startGame.disabled) return;
    if (window.isMultiplayer) {
      if (window.mpState === 'readyStart') {
        NET.startReady();
        window.mpState = 'waitingStart';
        els.startGame.textContent = 'Aguardando oponente iniciar...';
        els.startGame.disabled = true;
      } else if (!window.mpState) {
        NET.deckChoice(G.playerDeckChoice);
        if (window.opponentConfirmed) {
          window.mpState = 'readyStart';
          els.startGame.textContent = 'Iniciar';
          els.startGame.disabled = false;
        } else {
          window.mpState = 'waitingDeck';
          els.startGame.textContent = 'Aguardando oponente confirmar deck...';
          els.startGame.disabled = true;
        }
      }
    } else {
  ...
  els.rematchBtn.addEventListener('click', () => {
    ...
    if (window.isMultiplayer && window.NET) {
      ...
      els.endSub.textContent = 'Aguardando oponente';
    } else {
      els.endOverlay.classList.remove('show');
      startGame();
    }
  ...
  function handleMove(move) {
    switch (move.type) {
      case 'summon': {
        summon('ai', move.card, move.stance, true);
        applyBattlecryEffects('ai', move.effects || []);
        G.aiMana -= move.card.cost;
        renderAll();
        break;
      }
      case 'attack': {
        const a = G.aiBoard.find((x) => x.id === move.attackerId);
        const t = G.playerBoard.find((x) => x.id === move.targetId);
        if (a && t) attackCard(a, t);
        break;
      }
      case 'attackFace': {
        const a = G.aiBoard.find((x) => x.id === move.attackerId);
        if (a) attackFace(a, 'player');
        break;
      }
    }
  }
  ...
  if (window.NET) {
    NET.onOpponentDeckConfirmed((d) => {
      G.aiDeckChoice = d;
      if (window.mpState === 'waitingDeck') {
        els.startGame.textContent = 'Iniciar';
        els.startGame.disabled = false;
        window.mpState = 'readyStart';
      } else {
        window.opponentConfirmed = true;
      }
    });
    NET.onStartGame(() => {
      els.start.style.display = 'none';
      ...
      ensureRunning();
      stopMenuMusic();
      startGame(NET.isHost() ? 'player' : 'ai');
      window.mpState = null;
      window.opponentConfirmed = false;
    });
    NET.onOpponentName((n) => {
      window.opponentName = n;
      updateOpponentLabel();
    });
  }
  if (window.NET) {
    ...
    NET.onOpponentLeft(() => {
      ...
      if (window.isMultiplayer && els.wrap.style.display === 'block')
        endGame(true);
    });
  ...
  function applyTotemBuffsWithFx(t) {
    ...
    const picks = shuffle(G.playerBoard.slice()).slice(0, cnt);
    const fx =
      t.buffs && t.buffs.atk && t.buffs && t.buffs.hp
        ? 'magic'
        : t.buffs && t.buffs.atk
          ? 'attack'
          : 'healing';
    picks.forEach((u) => {
      ...
      if (t.buffs && t.buffs.hp) u.hp += t.buffs.hp;
      try {
        particleOnCard(u.id, fx);
      } catch (_) {}
      if (t.buffs && t.buffs.atk) {
        try {
          fxTextOnCard(u.id, `+${t.buffs.atk} ATK`, 'buff');
        } catch (_) {}
      }
      if (t.buffs && t.buffs.hp) {
        try {
          fxTextOnCard(u.id, `+${t.buffs.hp} HP`, 'buff');
        } catch (_) {}
      }
    });
  }
  function openTotemConfirm(anchor, onConfirm, onCancel) {
    ...
    bUse.addEventListener('click', (e) => {
      e.stopPropagation();
      cleanup();
      onConfirm && onConfirm(anchor);
    });
    ...
    setTimeout(() => {
      const h = (ev) => {
        if (ev.target.closest('.totem-confirm')) return;
        window.removeEventListener('click', h, true);
        cleanup();
      };
      window.addEventListener('click', h, true);
  ...
  const __orig_playFromHand =
    typeof playFromHand === 'function' ? playFromHand : null;
  function playFromHand(id, st) {
    if (G.current !== 'player') return;
    const i = G.playerHand.findIndex((c) => c.id === id);
    if (i < 0) return;
    ...
    if (isT) {
      ...
      const node =
        document.querySelector(`#playerHand .card[data-id='${c.id}']`) ||
        ...
        try {
          if (!window.animationsDisabled) {
            if (anchor) {
              ...
              ghost.classList.add('disintegrate');
              document.body.appendChild(ghost);
              setTimeout(() => {
                ghost.style.transform = 'scale(.85) rotate(-2deg)';
                ghost.style.opacity = '0';
              }, 10);
              setTimeout(() => {
                try {
            ...
            const bar = document.getElementById('totemBar');
            const slots = bar ? Array.from(bar.children) : [];
            const idx = G.totems.length;
            const slot = slots[idx];
            if (slot) {
              const sr = slot.getBoundingClientRect();
              const fly = document.createElement('div');
              fly.className = 'totem-fly';
              fly.textContent = t.icon || '🗿';
              Object.assign(fly.style, {
                left:
                  (anchor ? anchor.getBoundingClientRect().left : sr.left) +
                  'px',
                top:
                  (anchor ? anchor.getBoundingClientRect().top : sr.top) + 'px',
              });
              document.body.appendChild(fly);
              requestAnimationFrame(() => {
                fly.style.transform = `translate(${sr.left + sr.width / 2 - fly.getBoundingClientRect().left}px, ${sr.top + sr.height / 2 - fly.getBoundingClientRect().top}px) scale(0.7)`;
                fly.style.opacity = '1';
              });
              setTimeout(() => {
                try {
    ...
    const origin = document.querySelector(
      `#playerHand .card[data-id='${c.id}']`,
    );
    if (origin && !window.animationsDisabled) {
      try {
        flyToBoard(origin, () => {});
      } catch (_) {}
    }
  ...
  window._activateTotemByIdImpl = function (cardId, anchor) {
    ...
    t.desc = describeTotem(c);
    try {
      if (!window.animationsDisabled && anchor) {
        const r = anchor.getBoundingClientRect();
        ...
        requestAnimationFrame(() => {
          ...
          anchor.style.transform = `translate(${cx}px,${cy}px) scale(1.02)`;
        });
        setTimeout(() => {
          anchor.style.transform += ' scale(.85) rotate(-2deg)';
        ...
        if (slot) {
          ...
          document.body.appendChild(fly);
          requestAnimationFrame(() => {
            fly.style.transform = `translate(${sr.left + sr.width / 2 - (r.left + r.width / 2)}px, ${sr.top + sr.height / 2 - (r.top + r.height / 2)}px) scale(1)`;
            fly.style.opacity = '1';
          });
          setTimeout(() => {
            try {
...
function activateTotemById(cardId, anchor) {
  if (window._activateTotemByIdImpl)
    return window._activateTotemByIdImpl(cardId, anchor);
  const i = G.playerHand.findIndex((x) => x.id === cardId);
  if (i < 0) return;
  const c = G.playerHand[i];
  if (G.totems.length >= 3) {
