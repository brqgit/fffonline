  ...
  function renderEncy(filter = "all", locked = false) {
    ...
    cards.forEach((c) => {
      ...
      const kwChips = (c.kw || []).map(
        (k) => `<span class='chip' data-type='keyword' data-tip='${k === "Protetor" ? "Enquanto houver Protetor ou carta em Defesa do lado do defensor, ataques devem mir\xE1-los." : k === "Furioso" ? "Pode atacar no turno em que \xE9 jogada." : k === "Absorver" ? "Ao entrar, copia uma palavra-chave de um aliado." : k === "Mut\xE1vel" ? "No fim do turno, troca ATK e HP." : ""}' >${k}</span>`
      );
      if (c.subclasse && c.classe) {
        kwChips.push(`<span class='chip class-tag ${c.classe}'>${c.subclasse}</span>`);
      }
      const kwMarkup = kwChips.length ? `<div class='kw-tags'>${kwChips.join("")}</div>` : "";
      const effectMarkup = c.text ? `<p class='effect-text'>${c.text}</p>` : "";
      d.innerHTML = `<div class='bg bg-${c.deck}'></div><div class='head'><span class='cost'>${c.cost}${c.harvestCost ? `\u{1F33E}${c.harvestCost}` : ""}</span><div class='name'>${c.name}</div></div><div class='mini'>${c.tribe} \u2022 \u2694\uFE0F ${c.atk} / \u2764\uFE0F ${c.hp}</div><div class='art'>${art}</div><div class='details'>${kwMarkup}${effectMarkup}</div>`;
      tiltify(d);
