    floresta: [
      ...
      ['Ent Ancião','🌳','Animal',2,4,3,'Entra: ganha 1 de mana','', 'M1'],
      ['Xamã do Bosque','🪓🌲','Animal',2,2,3,'Entra: sacrifica um aliado e ganha o custo como mana','', 'SM']
    ]
  };
  ...
  const els={
    pHP:$('#playerHP'),pHP2:$('#playerHP2'),aHP:$('#aiHP'),aHP2:$('#aiHP2'),
    mana:$('#mana'),pHand:$('#playerHand'),pBoard:$('#playerBoard'),aBoard:$('#aiBoard'),
    endBtn:$('#endTurnBtn'),muteBtn:$('#muteBtn'),aAva:$('#aiAvatar'),
    drawCount:$('#drawCount'),discardCount:$('#discardCount'),
    barPHP:$('#barPlayerHP'),barAHP:$('#barAiHP'),barMana:$('#barMana'),
    wrap:$('#gameWrap'),start:$('#start'),openEncy:$('#openEncy'),ency:$('#ency'),
    encyGrid:$('#encyGrid'),encyFilters:$('#encyFilters'),closeEncy:$('#closeEncy'),
    startGame:$('#startGame'),endOverlay:$('#endOverlay'),endMsg:$('#endMsg'),
    endSub:$('#endSub'),playAgainBtn:$('#playAgainBtn'),rematchBtn:$('#rematchBtn'),
    menuBtn:$('#menuBtn'),openMenuBtn:$('#openMenuBtn'),gameMenu:$('#gameMenu'),
    closeMenuBtn:$('#closeMenuBtn'),resignBtn:$('#resignBtn'),restartBtn:$('#restartBtn'),
    mainMenuBtn:$('#mainMenuBtn'),turnInfo:$('#turnInfo'),turnBanner:$('#turnBanner'),
    emojiBar:$('#emojiBar'),playerEmoji:$('#playerEmoji'),opponentEmoji:$('#opponentEmoji')
  };
  ...
  function showMultiplayerDeckSelect(){els.wrap.style.display='none';els.start.style.display='block';const mpOpen=document.getElementById('openMultiplayer');mpOpen&&(mpOpen.style.display='none');const btn=document.getElementById('startGame');if(btn){btn.textContent='Confirmar deck';btn.disabled=false}window.isMultiplayer=true;window.mpState=null;window.opponentConfirmed=false;const custom=document.querySelector('.deckbtn[data-deck="custom"]');custom&&(custom.style.display='none')}

  function showTurnIndicator(){if(!els.turnInfo||!els.turnBanner)return;const txt=G.current==='player'?'Seu turno':'Turno do oponente';els.turnInfo.textContent=txt;els.turnBanner.textContent=txt;els.turnBanner.classList.add('show');setTimeout(()=>els.turnBanner.classList.remove('show'),1200)}
  ...
  function newTurn(){if(G.current==='player'){G.playerManaCap=clamp(G.playerManaCap+1,0,10);G.playerMana=G.playerManaCap;draw('player',1);G.playerBoard.forEach(c=>c.canAttack=true)}else{G.aiManaCap=clamp(G.aiManaCap+1,0,10);G.aiMana=G.aiManaCap;draw('ai',1);G.aiBoard.forEach(c=>c.canAttack=true)}renderAll();showTurnIndicator()}

  function endTurn(){if(G.current!=='player')return;G.current='ai';G.chosen=null;updateTargetingUI();newTurn();sfx('end');if(window.isMultiplayer){NET.sendTurn('end')}else{setTimeout(aiTurn,500)}}
  ...
  function summon(side,c,st='attack',skipBC=false){const board=side==='player'?G.playerBoard:G.aiBoard;c.stance=st;c.canAttack=(st==='attack')&&c.kw.includes('Furioso');board.push(c);log(`${side==='player'?'Você':'Inimigo'} jogou ${c.name} em modo ${st==='defense'?'defesa':'ataque'}.`);let effects=[];if(!skipBC){effects=triggerBattlecry(side,c);if(window.isMultiplayer&&side==='player'){NET.sendMove({type:'summon',card:c,stance:st,effects})}}if(st==='defense')setTimeout(()=>animateDefense(c.id),30);return effects}
  ...
  els.muteBtn.addEventListener('click',()=>{initAudio();ensureRunning();allMuted=!allMuted;musicMuted=allMuted;sfxMuted=allMuted;if(master) master.gain.value=allMuted?0:.18*sfxVolume; if(musicGain) musicGain.gain.value=allMuted?0:musicBase*musicVolume; els.muteBtn.textContent=allMuted?'🔇 Som':'🔊 Som'});
  window.addEventListener('keydown',e=>{if(e.key==='Escape')cancelTargeting()});
  document.addEventListener('click',e=>{if(!G.chosen)return;if(e.target.closest('#aiBoard .card.selectable')||e.target.closest('#playerBoard .card.selectable')||e.target.closest('#aiBoard .face-attack-btn'))return;cancelTargeting()},{capture:true});
  ...
  if(window.NET){NET.onOpponentDeckConfirmed(d=>{G.aiDeckChoice=d;if(window.mpState==='waitingDeck'){els.startGame.textContent='Iniciar';els.startGame.disabled=false;window.mpState='readyStart'}else{window.opponentConfirmed=true}});NET.onStartGame(()=>{els.start.style.display='none';els.wrap.style.display='block';initAudio();ensureRunning();stopMenuMusic();startGame(NET.isHost()?'player':'ai');window.mpState=null;window.opponentConfirmed=false})}
