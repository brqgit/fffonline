...
function tiltify(card){const h=card.offsetHeight,w=card.offsetWidth;card.addEventListener('mousemove',e=>{if(card.classList.contains('chosen'))return;const x=(e.offsetX/w-.5)*12,y=(e.offsetY/h-.5)*-12;card.style.transform=`perspective(600px) rotateX(${y}deg) rotateY(${x}deg)`});card.addEventListener('mouseleave',()=>{card.style.transform=''})}
