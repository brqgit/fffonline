...
(function () {
  ...
  const NET = {
    ...
    leave() {
      try { this.socket?.disconnect(); } catch (_) {}
        ...
        this.online = false; this._setStatus("Modo: Solo");
        window.dispatchEvent(new CustomEvent("net:mode", { detail: { online: false } }));
      }
    },
    ...
    _wireSocket() {
      ...
      s.on("disconnect", () => {
        this.online = false; this._setStatus("Desconectado");
        window.dispatchEvent(new CustomEvent("net:mode", { detail: { online: false } }));
      });
      ...
      s.on("host:ack", ({ room }) => {
        this._setStatus(`Hospedando sala ${room}`);
        this.online = true;
        window.dispatchEvent(new CustomEvent("net:mode", { detail: { online: true } }));
      });
      s.on("join:ack", ({ room }) => {
        this._setStatus(`Conectado à sala ${room}`);
        this.online = true;
        window.dispatchEvent(new CustomEvent("net:mode", { detail: { online: true } }));
      });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("click", (ev) => {
      ...
      NET.deckSelect(btn.getAttribute("data-deck"));
    });
  });
})();
